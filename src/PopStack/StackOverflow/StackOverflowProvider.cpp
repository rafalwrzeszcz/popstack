...
#include "StackOverflowProvider.hpp"
...
using PopStack::StackOverflow::ApiException;
    ...
    return client
        ...
        .then([](string content) -> value {
                ...
                if (data.has_field("error_message")) {
                    throw ApiException(data.at("error_message").as_string());
                }
...
task< optional< string > > StackOverflowProvider::search(string query) {
    return fetch("similar?order=desc&sort=relevance&title=" + uri::encode_uri(query))
