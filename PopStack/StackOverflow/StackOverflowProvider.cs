...
namespace PopStack.StackOverflow {
    class StackOverflowProvider : ProviderInterface {
        ...
        {
            HttpWebRequest request = WebRequest.Create(StackOverflowProvider.BuildRequestUrl(call)) as HttpWebRequest;
        ...
        {
            if (StackOverflowProvider.Snippet.IsMatch(content)) {
                Match match = StackOverflowProvider.Snippet.Match(content);
        ...
        public string search(string query)
        {
            ...
            JArray items = StackOverflowProvider.ParseResponse(
                StackOverflowProvider.MakeRequest("similar?order=desc&sort=relevance&title=" + HttpUtility.UrlEncode(query))
            ).Value<JArray>("items");
            JToken buffer;
            foreach (JToken token in items) {
                ...
                if (item.TryGetValue("accepted_answer_id", out buffer)) {
                    answer = StackOverflowProvider.ExtractSnippet(
                        StackOverflowProvider.ParseResponse(
                            StackOverflowProvider.MakeRequest("answers/" + buffer.ToString() + "?filter=withbody")
